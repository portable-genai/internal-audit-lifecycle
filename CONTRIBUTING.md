...
| # | File | What to add | Enforced by |
|---|---|---|---|
| 1 | `src/internal_audit_lifecycle/adapters/<family>/<port>.py` | The class. Exactly one constructor shape: `def __init__(self, settings: Settings)`. Cloud SDK imports go INSIDE the method that needs them. | `test_port_parity.py::test_adapter_constructs_with_a_single_settings_argument` |
| 2 | `src/internal_audit_lifecycle/config.py` | The `module:Class` target in `DEFAULT_BINDINGS[<port>][<profile>]`. | `test_port_parity.py::test_every_port_binds_every_known_profile` |
