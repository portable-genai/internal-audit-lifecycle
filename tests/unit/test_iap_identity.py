...
from internal_audit_lifecycle.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from internal_audit_lifecycle.ports.identity import (
    VERIFIED,
...
_API_MODULE = "internal_audit_lifecycle.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        "  control_results:",
        *[
            f"    {p}: {_PKG}.adapters.local.control_results:LocalControlResultsAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.control_results:OnPremControlResultsAdapter",
        "  horizon:",
        *[f"    {p}: {_PKG}.adapters.local.horizon:LocalHorizonAdapter" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.horizon:OnPremHorizonAdapter",
        "  knowledge_base:",
        *[
        ...
        "  finding_feed:",
        *[
            f"    {p}: {_PKG}.adapters.local.finding_feed:LocalFindingFeedAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.finding_feed:OnPremFindingFeedAdapter",
    ]
)
...
def test_an_emptied_audience_variable_refuses_at_load(
    ...
    ``${AUDIT_IAP_AUDIENCE:-}`` is expanded by the settings loader, and the loader refuses a
