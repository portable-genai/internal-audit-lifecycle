"""What the UI's model pill states first must be true of the profile the service is running.

Every served console shows, at the top right of every page, a pill naming WHICH model answers,
with WHERE it is running in its title, until an answer names the model that answered
(``X-Answered-By``, ``tests/unit/test_answer_provenance.py``). Both values come from
``/healthz`` because the browser cannot know either: a console that read its runtime from
...
def test_the_health_contract_carries_both_halves() -> None:
    ...
    """
    from internal_audit_lifecycle.api.schemas import HealthResponse
...
def test_not_implemented_is_claimed_only_by_an_adapter_that_never_calls_a_model() -> None:
    ...
    if settings.generator_model != "managed-not-implemented":
        return
    from internal_audit_lifecycle.config import _GENERATOR_PORT
