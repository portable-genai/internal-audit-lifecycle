# ARCHITECTURE: Internal Audit Lifecycle Copilot (`internal-audit-lifecycle`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`AUDIT_PROFILE`) swaps the
...
## Layout (`src/internal_audit_lifecycle/`)
