...
def _check_annual_plan(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if facts.get("top_entity") != "ent-payments" or facts.get("top_score") != 100:
        problems.append("the deterministic ranking did not put ent-payments (100) first")
    if facts.get("top_band") != "critical":
        problems.append("the top entity was not banded critical")
    if not facts.get("plan_routed"):
        problems.append("the consequential plan was not routed for sign-off (rule R8)")
    return problems
...
CHECKS: dict[str, Check] = {
    ...
    "annual_plan": _check_annual_plan,
