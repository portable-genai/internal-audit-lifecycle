...
The catalog id (``Aud1``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "internal_audit_lifecycle"
_OLD_ENV_PREFIX = "AUDIT_"
_OLD_BARE_PREFIX = "AUDIT"
_OLD_RESOURCE = "aud1-svc"
_OLD_DIST = "internal-audit-lifecycle"
_OLD_CATALOG_ID = "Aud1"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (AUDIT_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-audit",
    )
