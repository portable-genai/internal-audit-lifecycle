...
from internal_audit_lifecycle.domain.narration import (
...
class DemoRun:
    ...
    def _step_annual_plan(self) -> Produced:
        ...
        review = self.container.review_router.route(
            ...
            maker="demo.auditor@bank.example",
            tenant=TENANT,
        )
        ranking = Panel(
            title="Risk-based annual plan (deterministic ranking)",
            rows=tuple(
                Row(
                    "#" + str(entry.rank) + " " + entry.entity_id,
                    str(entry.score) + " (" + entry.band.value + ")",
                    "bad" if entry.band.value in ("critical", "high") else "",
                )
                for entry in plan.entries
            ),
            note="Score and band are pure code; the same universe always yields the same ranking.",
        )
        narrative = Panel(
            title="Model narration (grounded, discarded if it invents a figure)",
            rows=(
                Row("Narrative", note.text),
                Row("Model authored", "yes" if note.model_authored else "no (fallback)"),
                Row("Routed for sign-off", review, "ok"),
            ),
            note="The narrative restates only engine figures; a hallucinated number is discarded.",
            tone="ok",
        )
        top = plan.entries[0]
        facts = {
            "top_entity": top.entity_id,
            "top_score": top.score,
            "top_band": top.band.value,
            "plan_routed": bool(review),
            "entity_count": len(plan.entries),
        }
        return [ranking, narrative], facts
...
def _exit_control_results(container: Any) -> Any:
    return container.control_results.results_for("payments")


def _exit_horizon(container: Any) -> Any:
    return container.horizon.signals()


def _exit_knowledge_base(container: Any) -> Any:
    from internal_audit_lifecycle.domain.fieldwork import RetrievalQuery

    return container.knowledge_base.search(RetrievalQuery(area="payments", text="q"))


def _exit_finding_feed(container: Any) -> Any:
    from internal_audit_lifecycle.domain.findings import IssueHandover

    return container.finding_feed.emit(
        IssueHandover(
            finding_id="f",
            engagement="e",
            area="payments",
...
EXIT_CALLS: dict[str, Callable[[Any], Any]] = {
    ...
    "finding_feed": _exit_finding_feed,
}
