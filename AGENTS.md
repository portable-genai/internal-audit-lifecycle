...
- `src/internal_audit_lifecycle/domain/` is PURE stdlib. No web framework, no cloud SDK,
...
- **The exposure guard is derived from the IDENTITY BINDING, and from nothing else.** An end-user
  ...
  `AUDIT_S2S_TOKEN` may never enter that decision: it authenticates a
...
- **The one adapter that declares `VERIFIED` must EARN it, and it is the one adapter that may not
  ...
  configured `AUDIT_IAP_AUDIENCE`, three-state: unset or emptied REFUSES,
...
- **One profile read, three states, refused at import.** `config.PROFILE_CHOICE` resolves once at
  ...
  `AUDIT_PROFILE` (mentioning it in a refusal message is wanted, reading
