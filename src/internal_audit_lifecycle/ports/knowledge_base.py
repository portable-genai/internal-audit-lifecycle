...
Fieldwork grounds its working-paper drafts against retrieved evidence, and Aud1 does NOT build a
retrieval backend of its own: the workpaper and prior-audit corpus is ingested into the shared
governed store and retrieved from it (governed RAG). The ``local`` adapter answers from a
deterministic fixture corpus, the ``gcp`` adapter reads Gemini API File Search over the corpus
(lazy, refuses when unconfigured), and ``onprem`` fails fast. Empty retrieval is a legitimate
answer the fieldwork engine turns into "no draft"; a failure to REACH the store is a raised error.
