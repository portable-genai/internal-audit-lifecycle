...
One env var (``AUDIT_PROFILE``) selects the adapter family for every
...
from .ports.control_results import ControlResultsReadPort
from .ports.finding_feed import FindingFeedPort
from .ports.generation import GenerationPort
from .ports.horizon import HorizonReadPort
...
_PROFILE_ENV = "AUDIT_PROFILE"
_SETTINGS_ENV = "AUDIT_SETTINGS"
...
#: The name the ``local`` deterministic generator answers under: what ``generator_model`` reports
#: under ``local`` and what that adapter notes as the model that answered (``X-Answered-By``), so
#: one stub has one name on the pill before and after an answer.
...
class ProfileChoice:
    ...
    The variable is ``AUDIT_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("AUDIT_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "control_results": {
        "local": f"{_PKG}.adapters.local.control_results:LocalControlResultsAdapter",
        "gcp": f"{_PKG}.adapters.gcp.control_results:CloudControlResultsAdapter",
        "onprem": f"{_PKG}.adapters.onprem.control_results:OnPremControlResultsAdapter",
    },
    "horizon": {
        "local": f"{_PKG}.adapters.local.horizon:LocalHorizonAdapter",
        "gcp": f"{_PKG}.adapters.gcp.horizon:CloudHorizonAdapter",
        "onprem": f"{_PKG}.adapters.onprem.horizon:OnPremHorizonAdapter",
    },
    "knowledge_base": {
    ...
    "finding_feed": {
        "local": f"{_PKG}.adapters.local.finding_feed:LocalFindingFeedAdapter",
        "gcp": f"{_PKG}.adapters.gcp.finding_feed:CloudFindingFeedAdapter",
        "onprem": f"{_PKG}.adapters.onprem.finding_feed:OnPremFindingFeedAdapter",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``AUDIT_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "AUDIT_REVIEW_ROUTING"
...
class Settings:
    ...
    def generator_model(self) -> str:
        ...
        calls: under ``gcp`` the setting its call reads, never a model a flag could swap in, and
        under ``local`` :data:`LOCAL_STUB_MODEL`, the name the local stub notes when it answers.
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            control_results_url=str(data.get("control_results_url") or ""),
            horizon_url=str(data.get("horizon_url") or ""),
            knowledge_base_url=str(data.get("knowledge_base_url") or ""),
            aud3_feed_url=str(data.get("aud3_feed_url") or ""),
...
class Container:
    ...
    @cached_property
    def control_results(self) -> ControlResultsReadPort:
        adapter = self._bind("control_results")
        assert isinstance(adapter, ControlResultsReadPort)
        return adapter

    @cached_property
    def horizon(self) -> HorizonReadPort:
        adapter = self._bind("horizon")
        assert isinstance(adapter, HorizonReadPort)
        return adapter
    ...
    @cached_property
    def finding_feed(self) -> FindingFeedPort:
        adapter = self._bind("finding_feed")
        assert isinstance(adapter, FindingFeedPort)
        return adapter
