...
The client wires its own obligations-control-mapping link behind this seam. Until then it refuses at
call time rather than pretending, so a placeholder never becomes a silent empty answer that would
...
class OnPremObligationsAdapter:
    ...
    obligations-control-mapping link.
    """
    ...
    def obligations_for(self, area: str) -> tuple[ObligationRef, ...]:
        raise NotImplementedError(
            ...
            "obligations-control-mapping connection (see docs/onprem-migration.md)"
        )
