"""GCP HorizonPort adapter: reach compliance-advisory over HTTPS, refusing when unconfigured.
...
class CloudHorizonAdapter:
    """Reach compliance-advisory over its authenticated HTTPS surface, or
    refuse when unconfigured.
    """
    ...
    def signals(self) -> tuple[HorizonSignal, ...]:
        ...
        if not url:
            raise RuntimeError(
                "compliance-advisory endpoint is unconfigured: set AUDIT_HORIZON_URL to the "
                "deployment "
    ...
    def _reach(
        self, url: str
    ) -> tuple[HorizonSignal, ...]:  # pragma: no cover - needs live compliance-advisory
    ...
    def _parse(
        ...
        raise NotImplementedError(
            "wire compliance-advisory payload parsing when its live surface is available"
        )
