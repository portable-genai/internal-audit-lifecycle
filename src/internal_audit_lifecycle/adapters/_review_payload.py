...
def _kit_citations(result: TriageResult) -> tuple[KitCitation, ...]:
    ...
    A locator is routinely built from client text (``case:<the case subject>``), so masking only
    the snippet let the identifier cross to the shared console in the field named like a key.
    De-duplication keys off the REDACTED locator, so two cases that differ only in a masked
...
def result_to_review(result: TriageResult, *, maker: str, tenant: str = "") -> Review:
    ...
    content. A case subject should never carry personal data; the redaction has to hold whether
    or not it does.
    ...
    national id is not a subject anyone should be keying on.
    """
    subject = _redact(result.subject)
    return Review(
        action="internal_audit_lifecycle:triage",
        subject=subject,
        maker=maker,
        tenant=tenant,
        ...
        sod_group="internal_audit_lifecycle-maker-checker",
        case_ref=subject,
        # Producer-owned, tenant-scoped key so a retried delivery is idempotent at the console.
        source_key=f"internal-audit-lifecycle:{subject}:{result.severity.value}",
