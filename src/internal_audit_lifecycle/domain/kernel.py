...
def redacted_citations(citations: tuple[Citation, ...]) -> tuple[Citation, ...]:
    ...
    case subject>``), so both are raw client text wearing a structural-looking name. No sink can
    tell an engine-built citation from an extractor-built one by inspection, so this masks
