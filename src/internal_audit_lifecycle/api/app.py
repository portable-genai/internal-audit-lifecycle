...
``AUDIT_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``AUDIT_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from ..domain.authz import CrossTenantError, authorize_estate_access
from ..domain.fieldwork import RetrievalQuery, WorkpaperService
from ..domain.findings import FindingInput, FindingService, handover_envelope
...
from ..domain.scoping import SampleUnit, ScopingService, stratified_sample
...
_TOKEN_ENV = "AUDIT_S2S_TOKEN"
_INSECURE_DEMO_ENV = "AUDIT_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "AUDIT_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "AUDIT_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "AUDIT_S2S_AUDIENCE"
_API_HOST_ENV = "AUDIT_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``AUDIT_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``AUDIT_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "AUDIT_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "internal-audit-lifecycle"
...
_APP_TITLE = "Internal Audit Lifecycle Copilot"
...
# pill names is what answered, never what configuration says would. The `generation` adapters
# note: the Gemini one names the model it called, the local stub `LOCAL_STUB_MODEL`; nothing here
# attaches a search tool. A request that called no model (scoping, findings) sends neither header,
# and the pill keeps showing the configured `generator_model` from `/healthz`.
...
#
# Note what is NOT in this expression: AUDIT_S2S_TOKEN. A service
...
def _envelope(
    *, subject: str, severity: object, decision: object, summary: str, citations: object
) -> TriageResult:
    """Project a consequential audit result onto the shared R8 review envelope.

    The review path is shared across the fleet and speaks the kernel result shape, so a plan or a
    finding is projected onto it here, at the surface, rather than the pure domain importing a
    surface type. ``requires_human_review`` is always True: only consequential results reach here.
    """
    return TriageResult(
        subject=subject,
...
@app.post("/v1/plan", response_model=PlanResponse, tags=["artifacts"])
def plan(
    request: PlanRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> PlanResponse:
...
@app.post("/v1/scope", response_model=ScopeResponse, tags=["artifacts"])
def scope(
    ...
    _authorize(principal)
    container = _container()
    obligations = tuple(container.obligations.obligations_for(request.area))
    results = tuple(container.control_results.results_for(request.area))
    engagement_scope = ScopingService().build_scope(request.area, obligations, results)
    # The sampling population: the failing controls plus the obligations, one sampleable unit each.
    units = tuple(
        SampleUnit(id=cid, stratum="control") for cid in engagement_scope.failing_controls
    ) + tuple(SampleUnit(id=o.id, stratum="obligation") for o in engagement_scope.obligations)
    sample = stratified_sample(
        units, seed=request.seed, as_of=request.as_of, size=request.sample_size
    )
    return ScopeResponse.from_domain(engagement_scope, sample)


@app.post("/v1/workpaper", response_model=WorkpaperResponse, tags=["artifacts"])
def workpaper(
    request: WorkpaperRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> WorkpaperResponse:
    """Draft a grounded working paper from retrieved evidence, or decline when retrieval is empty.

    The corpus is read through the ``knowledge_base`` port; the model drafts only from retrieved
    passages and the draft is discarded unless every citation is a retrieved source. Empty
    retrieval means NO draft: the auditor is told the corpus had nothing, never handed an
...
@app.post("/v1/finding", response_model=FindingResponse, tags=["artifacts"])
def finding(
...
def finding_handover(
    ...
    envelope = handover_envelope(result, approval_ref=request.approval_ref)
    feed_ref = container.finding_feed.emit(envelope)
...
def _authorize(principal: Principal) -> None:
    """Authorise the verified principal's tenant against the audit estate, or 403 (never 404)."""
    try:
        authorize_estate_access(principal.tenant)
    except CrossTenantError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc


def _record(container: Container, *, action: str, result: object, actor: str) -> None:
    """Write one already-redacted audit event for a consequential engine result.

    The summaries are engine-derived and carry no PII, but the rule is unconditional: nothing
    reaches the WORM record that did not pass through the audit envelope.
    """
    ...
    container.audit.record(
        AuditEvent(
            ...
            redacted_summary=str(getattr(result, "summary", "")),
