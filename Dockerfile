...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    AUDIT_PROFILE=gcp \
    AUDIT_SETTINGS=/app/config/settings.yaml \
    PORT=8080
...
CMD ["sh", "-c", "python -m internal_audit_lifecycle.managed_readiness && exec uvicorn internal_audit_lifecycle.api.app:app --host 0.0.0.0 --port ${PORT:-8080}"]
